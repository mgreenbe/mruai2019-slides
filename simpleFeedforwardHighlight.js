...
let fig = d3.select("#simple-feedforward-highlight");
let diagram = fig.select("g.diagram");
let labels = fig.select("g.labels");

let width = fig.attr("width") - 100;
let height = fig.attr("height") - 100;
  ...
  .each(function(cs, i) {
    d3.select(this)
      ...
      .each(function(c, j) {
        ...
        if (i == 0) {
          d3.select(this)
            .selectAll("line.fragment")
            .data(data)
            .enter()
            .append("line")
            .attr("class", (_, k) => `fragment line-${j}-${k} current-only`)
            .attr("data-fragment-index", (_, k) => {
              console.log(j, j - 1, (j - 1) % 3);
              return 3 * ((j + 3 - 1) % 3) + k;
            })
            .attr("x1", c[0])
            .attr("y1", c[1])
